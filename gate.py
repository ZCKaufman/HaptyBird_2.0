...
class Gate:
    ...
    def generate_gate(self, params):
        ...
        if(self.p_gate_start < params["game_width"] - (params["gate_size"] + params["gate_cushion"])): 
        ...
        if(left_right < 5): # LEFT
            if(gate_loc[0] >= (params["gate_cushion"] + params["gate_size"] + params["gate_min_distance"])): # Expand right
                self.n_gate_start = random.randint((params["gate_cushion"] + params["gate_size"]), gate_loc[0] - params["gate_min_distance"])
                # Changes negative gate vlaues to -1
                self.gate_arr[self.n_gate_start - params["gate_size"]:self.n_gate_start] = -1
            else:
                self.n_gate_start = random.randint(gate_loc[-1] + params["gate_min_distance"], params["game_width"] - params["gate_cushion"])
                self.gate_arr[self.n_gate_start:self.n_gate_start + params["gate_size"]] = -1
        else: # RIGHT
            if(gate_loc[-1] <= params["game_width"] - (params["gate_cushion"] + params["gate_size"] + params["gate_min_distance"])): # Expand right
                self.n_gate_start = random.randint((gate_loc[-1] + (params["gate_min_distance"]  )), params["game_width"] - (params["gate_cushion"]))
                # Changes negative gate vlaues to -1
                self.gate_arr[self.n_gate_start: self.n_gate_start + (params["gate_size"]  )] = -1
            else:
    ...
    def display(self, params, game):
        ...
        if(self.neg_gate[0][0] < self.pos_gate[0][0]):
            ...
            pygame.draw.rect(game.surface, (255,255,0), (
                ...
                self.y  , # Start (y) wherever the gate is supposed to be
                self.neg_gate[0][0], # Have a width that spans from 0 to the beginning of the -gate
                5  , # Arbitrary height
            ))
            # Neg score
            font_bold = pygame.font.SysFont("Segoe UI", 5  , True)
            neg_gate_score = font_bold.render("$1.00", True, (255,0,0))
            game.surface.blit(neg_gate_score, (np.mean(self.neg_gate) - (5  ),self.y  ))
            # Middle wall
            pygame.draw.rect(game.surface, (255,255,0), (
            ...
            game.surface.blit(pos_gate_score, (np.mean(self.pos_gate) - (5  ),self.y  ))
            # Right wall
            pygame.draw.rect(game.surface, (255,255,0), (
